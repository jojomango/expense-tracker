...
**金額輸入的重大行為改變（請仔細看,可能是需要人類決策的事）：**

UI-SPEC.md §5 的數字鍵台鍵序是 `1 2 3 / 4 5 6 / 7 8 9 / 00 0 ⌫`——**沒有小數點鍵**。
這代表 Phase 9 之後，記帳頁**只能輸入整數金額**（例如 TWD 只能記到「元」，
不能再記「角、分」），送出時仍然呼叫 Phase 1 就有的 `parse(amountDigits, wallet.currency)`
把輸入字串當「整數位」解析（小數位自動補 0）。這是 UI-SPEC.md 明確畫出來的鍵台
（不是我自己省略小數點鍵），所以我沒有停下來，但有兩個連帶影響必須記錄：

1. **編輯一筆 Phase 9 之前建立、帶小數的舊交易時，金額會被「就地四捨五入」到整數**。
   `TransactionForm` 的 `digitsFromAmount(amount, currency)` 用 `Math.round` 把
   最小單位金額換算回整數位字串（例如 TWD 12050 分 = 120.50 元 → 顯示 `121`）。
   使用者只要打開編輯畫面又按下「儲存」（就算完全沒改金額），這筆交易的金額就會
   從 120.50 悄悄變成 121.00。目前**沒有任何測案覆蓋這個情境**（T7.3／T8.1／T8.2
   都只測整數輸入），純粹是我實作時發現的真實資料風險。
2. 這不是 TESTCASES.md 或 UI-SPEC.md 的矛盾——鍵台長相是 UI-SPEC.md §5 明確畫出來的，
   我沒有自作主張拿掉小數點鍵，只是照著畫的做。但這是一個**產品行為的實質改變**
   （記帳精度從「分」退化成「元」），建議人類決定：
   - 這是可以接受的取捨（多數人記帳不需要分），還是
   - 之後要在鍵台補一個小數點鍵（那會動到 UI-SPEC.md，需要人類先改契約）
...
- **上面提到的「編輯舊資料會四捨五入金額」是最重要的一個，請 Phase 10 開工前
  務必看過、也建議在 PR 描述的「需要人類決策」重新提出讓人類確認過一次。**
...
- **群組小計沒有任何測案覆蓋**（見上方設計決策），Phase 10 動 `Stats.tsx`／
  ...
  不是鎖死的契約。
- **`digitsFromAmount` 對「金額小於一個整數單位」的舊交易**（例如 TWD 只有
  1 分 = 最小單位 1）會算出 `major = 0`，顯示為空字串（等同「尚未輸入」），
  编輯這筆交易時主鍵會被停用（`isZero` 為真），使用者完全無法在不變更金額的
  情況下單純修改分類或日期再儲存——會卡住。這是上面「四捨五入」問題的極端版本，
  同樣沒有測案覆蓋，一併記錄。
...
金額輸入精度那項不是矛盾（UI-SPEC.md 的鍵台圖示就是沒有小數點鍵），但是一個
值得人類知道、且可能想介入的產品行為改變，已在上方「金額輸入的重大行為改變」
段落詳細記錄，也會寫進這個 phase 的 PR 描述。
